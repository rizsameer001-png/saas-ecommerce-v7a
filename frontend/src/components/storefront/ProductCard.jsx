...
export default function ProductCard({ product, storeSlug, currency = '$' }) {
  ...
  const safeStoreSlug = storeSlug || product.store?.slug

    if (!safeStoreSlug) {
      console.error("Store slug missing", product)
      return
    }
  ...
  // 🔴 FIX #1: SAFE fallback (prevents /store/undefined)
  const safeStoreSlug = storeSlug || product.store?.slug || ''

  const productLink = safeStoreSlug
    ? `/store/${safeStoreSlug}/products/${product.slug || product._id}`
    : `/marketplace/products/${product.slug || product._id}`

  return (
    <Link
      to={productLink}   // 🔴 FIXED HERE
      className="group bg-white rounded-2xl overflow-hidden shadow-card hover:shadow-card-hover transition-all duration-300 hover:-translate-y-0.5 flex flex-col"
    >
            ...
            <div className="flex">
              {[1,2,3,4,5].map(s => (
                <Star
                  ...
                  size={10}
                  className={
                    s <= Math.round(product.stats.rating)

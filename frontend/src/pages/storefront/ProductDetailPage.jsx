            ...
            <div className={`w-9 h-9 rounded-full flex items-center justify-center border-2 transition-colors ${
              ...
              {v.imageUrl ? (
                ...
                >
                  {isSel && (
                    <Check size={12}
                      className={parseInt((v.colorHex || '#ffffff').replace('#', ''), 16) > 0xffffff / 2 ? 'text-black' : 'text-white'}
                    />
                  )}
                </div>
              )}
...
export default function ProductDetailPage() {
  ...
  const colorOptions = useMemo(() => {
    if (!product) return []
    // From reusable productAttributes (color_swatch or image_swatch preferred)
    ...
    // Fallback: simple attributes.color → no hex, just names
    return (product.attributes?.color || []).map(c => ({ label: c, value: c.toLowerCase(), colorHex: null, imageUrl: null }))
  }, [product])
